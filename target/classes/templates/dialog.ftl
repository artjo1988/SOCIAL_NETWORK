...
<body onload="doConnect(), getMessages()">
<!-- Header -->
        ...
        <div class="flexWrap profileBlock">
            ...
            <div class="flexItem infoCollumnUsers">
                <#--<div class="white-bg dopBlock">-->
                    <div style="border-bottom: 2px solid rgba(202, 194, 199, 0.96); margin: -20px 0 15px 0; padding: 0 0 15px 0 ">
                        <span><a href="/users/${chat.candidate.id}"><img class="img-circle text-center m-t-15" width='50' height='50' alt="" src="${chat.urlAva}"></a></span>
                        <span style="font-size: 15px;"><a href="/users/${chat.candidate.id}"><span style="margin: 10px;"><strong>${chat.name}</strong></span></a></span>
                    </div>
                    <div class="chat_container">
                        <div class="form-group m-b-15 m-t-20">
                            <div class="col-lg-12" style="margin-left: -15px; height: 115px; width: 104%">
                                <textarea style="border-color: #c6c6c6" class="form-control " id="message" placeholder="Введите сообщение" name="message" aria-required="true" cols="75" rows = "1" maxlength = "75" autocomplete="on" wrap="hard" required
                                    onkeydown="if (event.keyCode == 13) {sendMessageBySocket(document.getElementById('message'), getUrlVars()['id']); return false; }"></textarea>
                            </div>
                            <div class="input-group-btn" >
                                <button class="btn btn-primary" style="float: right" id="send" onclick="sendMessageBySocket(document.getElementById('message'), getUrlVars()['id'])">
                                    Отправить
                                </button>
                            </div>
                        </div>
                        <select id="chatMessagesList" size="30" disabled style="width: 100%;"></select>
                    </div>
                <#--<#if chats??>-->
                    <#--<#list chats as chat>-->
                        <#--<div  style="margin-bottom: 15px ; display: block">-->
                            <#--<div style="display: flex;">-->
                                <#--<div>-->
                                    <#--<a class="dropdown-toggle" href="/users/${chat.candidate.id}"> <img class="img-circle text-center m-t-15" width='75' height='75' alt="" src="${chat.urlAva}">-->
                                <#--</div>-->
                                <#--<div style="padding: 40px 0 0 3%">-->
                                    <#--<p>-->
                                        <#--<a href="/users/${chat.candidate.id}"> <strong>${chat.name}</strong></a>-->
                                    <#--</p>-->
                                    <#--&lt;#&ndash;<a href="/users/${user.id}/message"><em>Написать сообщение</em></a>&ndash;&gt;-->
                                <#--</div>-->
                                <#--<div style="padding: 30px; margin-left: 200px;">-->
                                    <#--<#if user.role == "ADMIN">-->
                                        <#--<a href="#" >Открыть диалог</a>-->
                                    <#--<#else>-->
                                        <#--<a href="/chats/${chat.id}/" class="btn btn-purple w-md">Открыть диалог</a>-->
                                    <#--</#if>-->
                                <#--</div>-->
                            <#--</div>-->
                        <#--</div>-->
